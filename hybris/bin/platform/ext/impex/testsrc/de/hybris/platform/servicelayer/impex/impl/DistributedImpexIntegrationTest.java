...
public class DistributedImpexIntegrationTest extends ServicelayerBaseTest
{
	...
	public void shouldKeep_SLD_ENABLED_Flag_FALSE_WhenLegacyPersistenceIsSetTo_TRUE_Globally() throws Exception
	{
		...
		// when
		importService.importData(config);
		final DistributedImportProcessModel process = asDistributedImportProcess(
				distributedProcessService.wait("TEST_PROCESS", 1));

		// then
		assertThat(process).isNotNull();
		ImportMetadataAssert.assertThat(process.getMetadata()).hasSize(3);
		ImportMetadataAssert.assertThat(process.getMetadata())
		                    .containsMetadataKey(ImportBatchHandler.PROCESS_CODE)
		                    .withValue("TEST_PROCESS");
		ImportMetadataAssert.assertThat(process.getMetadata())
		                    .containsMetadataKey(ImportBatchHandler.SLD_ENABLED)
		                    .withValue("false");
		ImportMetadataAssert.assertThat(process.getMetadata())
		                    .containsMetadataKey(ImportBatchHandler.CODE_EXECUTION_FLAG)
		                    .withValue("false");
	}
	...
	public void shouldSwitchAllBatchesInto_IMPORT_BY_LINE_ModeWhenExceptionDuringBatchProcessingOccurs() throws Exception
	{
		...
		importService.importData(config);
		final DistributedImportProcessModel process = asDistributedImportProcess(
				distributedProcessService.wait("TEST_PROCESS", 5));
		...
		inputBatches.subList(1, 5).stream().map(ImportBatchModel::getMetadata).forEach(m -> ImportMetadataAssert.assertThat(m)
		                                                                                                        .containsOnlyMetadataKey(
				                                                                                                        ImportBatchHandler.IMPORT_BY_LINE_FLAG)
		                                                                                                        .withValue(
				                                                                                                        "true"));
