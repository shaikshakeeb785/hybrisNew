...
public class FlexibleSearchTTLTest extends HybrisJUnit4Test
{
	...
	public void testBothKeysHaveTtls()
	{
		...
		final StandardSearchResult<Product> second = (StandardSearchResult<Product>) flexibleSearchSecond.search(ctxWithTtl,
				query,
				params,
				Product.class);
		...
		final StandardSearchResult<Product> fourth = (StandardSearchResult<Product>) flexibleSearchFourth.search(ctxWithTtl,
				query,
				params,
				Product.class);
	...
	public void testAgainstKeysWithTtl()
	{
		...
		final FlexibleSearch flexibleSearchThird = createFlexibleSearchWithFixedTimeStamp(
				99999L); //no matter the time span since generation the same
	...
	public void testCachEntriesUnlessGenerationChanges()
	{
		...
		final FlexibleSearch flexibleSearchSecond = createFlexibleSearchWithFixedTimeStamp(
				99999L); //no matter the time span since generation the same
	...
	public void testAgainstKeysWithGenerationOnly()
	{
		...
		final FlexibleSearch flexibleSearchFifth = createFlexibleSearchWithFixedTimeStamp(
				99999L); //no matter the time span since generation the same
	...
	abstract static class FixedTimeFlexibleSearchCacheKey extends FlexibleSearchCacheKey
	{
		...
		public FixedTimeFlexibleSearchCacheKey(final TranslatedQuery tq, final Map values, final PK langPK,
		                                       final List<Class<?>> resultClasses, final boolean dontNeedTotal, final int start,
		                                       final int count,
		                                       final Set<Integer> beanTCs, final boolean executeQuery, final int ttlPeriod,
	...
	private FlexibleSearch createFlexibleSearchWithFixedTimeStamp(final long timestamp)
	{
		...
		return new FlexibleSearch()
		{
			...
			@Override
			FlexibleSearchCacheKey createCacheKey(final List<Class<?>> resultClasses, final boolean dontNeedTotal,
			                                      final int start,
			                                      final int count,
			                                      final de.hybris.platform.persistence.flexiblesearch.TranslatedQuery tQuery,
			                                      final boolean doExecuteQuery, final de.hybris.platform.core.PK langPK,
			                                      final java.util.Map values,
			                                      final java.util.Set<Integer> beanTCs, final int ttl, final List<Hint> hints)
			{
				return new FixedTimeFlexibleSearchCacheKey(tQuery, values, langPK, resultClasses, dontNeedTotal, start, count,
						beanTCs, doExecuteQuery, ttl, tenantid, hints)
				{
					@Override
					long getCurrentTime()
					{
