...
public class SessionContextTest extends HybrisJUnit4Test
{
	...
	public void testAttributeSet()
	{
		...
		final String attrName = "TEST1";
		final String someNiceValue = "someNiceValue";

		final Map<String, Object> sessionAttributes = new HashMap<>();
		sessionAttributes.put(attrName, someNiceValue);

		assertThat((String) sc.getAttribute(attrName)).isNull();

		try (SessionContext.SessionContextAttributeSetter attributeSetter = SessionContext.setSessionContextAttributesLocally(sc,
				sessionAttributes))
		{
			assertThat((String) sc.getAttribute(attrName)).isEqualTo(someNiceValue);
		}
	...
	public void testAttributeSetWithNullContext()
	{
		...
		try (SessionContext.SessionContextAttributeSetter attributeSetter = SessionContext.setSessionContextAttributesLocally(null,
				sessionAttributes))
		{
	...
	public void testAttributeSetWithNullContextAttributes()
	{
		...
		try
		{
			try (SessionContext.SessionContextAttributeSetter attributeSetter = SessionContext.setSessionContextAttributesLocally(
					null,
					null))
			{
			}
		...
		{
			assertThat(e).hasMessage("contextAttributes cannot be null");
	...
	public void testAttributeSetAndRestored()
	{
		...
		final String someNiceValue = "someNiceValue";
		final String attrName = "TEST1";
		sc.setAttribute(attrName, oldValue);

		final Map<String, Object> sessionAttributes = new HashMap<>();
		sessionAttributes.put(attrName, someNiceValue);

		assertThat((String) sc.getAttribute(attrName)).isEqualTo(oldValue);

		try (final SessionContext.SessionContextAttributeSetter attributeSetter = SessionContext.setSessionContextAttributesLocally(
				sc, sessionAttributes))
		{
			assertThat((String) sc.getAttribute(attrName)).isEqualTo(someNiceValue);
		}
...
//
			...
			org.junit.Assert.assertTrue("not all test threads shut down orderly",
					randomAccessHolder.waitAndDestroy(WAIT_SECONDS));
