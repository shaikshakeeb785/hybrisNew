...
public class DefaultModelServiceIntegrationTest extends ServicelayerBaseTest
{
	...
	public void testUniqueKeyInterceptor()
	{
		...
		try
		{
			...
			assertEquals(new HashSet<>(Arrays.asList(Unit.CODE, Unit.UNITTYPE)), typeService.getUniqueAttributes("Unit"));
	...
	public void testSetAttributeLocalizedWithSessionLang()
	{
		...
		modelService.setAttributeValue(lang, TitleModel.NAME, Collections.singletonMap(Locale.GERMANY, "bla"));
			...
			throws Exception
	{
		...
		// when
		final Map<Locale, Object> values = modelService.getAttributeValues(title, TitleModel.NAME, Locale.ITALIAN,
				Locale.CHINESE);
	...
	public void testSetAttributeLocalizedWithoutSessionLang()
	{
		...
		final Map<Locale, Object> params = new HashMap<>();
	...
	private class MyTestThread extends RegistrableThread
	{
		...
		public MyTestThread(final Tenant tenant, final ModelService modelService, final List<Boolean> semaphore,
		                    final PK pkToLock)
		{
			super();
			this.tenant = tenant;
	...
	public void testLockPK()
	{
		...
		modelService.save(lang);
		final List<Boolean> semaphore = Collections.synchronizedList((new ArrayList<>()));
