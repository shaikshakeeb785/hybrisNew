...
public class SessionServiceConcurrencyTest extends ServicelayerTransactionalBaseTest
{
	...
	private final Set<Integer> result = Collections.synchronizedSet(new HashSet<>());
	...
	public void testMultiThreadedGetOrCreate()
	{
		...
		final de.hybris.platform.test.RunnerCreator<Runnable> runnerCreator = new RunnerCreator<Runnable>()
		{
			@Override
				...
				return new Runnable()
				{
					...
					public void run()
					{
						try
						{
							...
							result.add(sessionService.getOrLoadAttribute(TEST_KEY,
									new SessionService.SessionAttributeLoader<Integer>()
									{
										@Override
										public Integer load()
										{
											LOG.warn(
													"Thread " + threadNumber + " load for session " + sessionService);
		...
		final TestThreadsHolder<Runnable> threads = new TestThreadsHolder<>(THREADS, runnerCreator);
