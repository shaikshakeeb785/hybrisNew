...
public class LazyLoadItemListUnitTest
{
	 ...
	 * wrapping underlying PKs into Longs.
	 */
	@Test
	                                  ...
	                                  final boolean listUsesSynchronization)
	{
		...
		final List<PK> pks = new ArrayList<>(PK_COUNT);
		...
		final LazyLoadItemList<Long> testList = new LazyLoadItemList<Long>(null, pks, PAGE_SIZE)
		{
			...
			{
				return listUsesSynchronization ? super.switchBufferedPageSynchronized(totalIndex)
						: super.switchBufferedPageNoLock(totalIndex);
			}
			...
			{
				final List<Long> page = new ArrayList<>(pks.size());
				for (final PK pk : pks)
				{
		...
		//random access test
		final TestThreadsHolder<LazyListRandomAccessRunner> randomAccessHolder = new TestThreadsHolder<>(//
				THREADS, //
				new de.hybris.platform.test.RunnerCreator<LazyListRandomAccessRunner>()
				{
					@Override
					public LazyListRandomAccessRunner newRunner(
							final int threadNumber)
					{
						final int startIdx = threadNumber * PAGE_SIZE;
						return new LazyListRandomAccessRunner(
								startIdx,
								PAGE_SIZE,
								TURNS,
								testList);
		...
		//ordered test
		final TestThreadsHolder<LazyListRandomAccessRunner> orderedHolder = new TestThreadsHolder<>(//
				THREADS, //
				new de.hybris.platform.test.RunnerCreator<LazyListRandomAccessRunner>()
				{
					@Override
					...
					{
						final int startIdx = threadNumber * PAGE_SIZE;
						return new LazyListConsecutiveAccessRunner(
								startIdx,
								PAGE_SIZE,
								TURNS,
								testList);
	...
	static private class LazyListRandomAccessRunner implements Runnable
	{
		...
		public void run()
		{
			...
			final List<LazyListRunnerError> recordedErrorTurns = new LinkedList<>();
