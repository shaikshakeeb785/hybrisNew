...
public class TenantRestartTest extends AbstractTenantInitializationTest
{
	...
	private static final String MESSAGE_LISTENER_DEF = ""
	...
	public void testJaloSessionObsoleteAfterTenantRestart() throws JaloSecurityException, ConsistencyCheckException
	{
		...
		session.getSessionContext().setLocale(Locale.getDefault());
		...
		Assertions.assertThat(JaloSession.assureSessionNotStale(session))
		          .overridingErrorMessage("Session should not be stale (no change in item version marker in db was stored)").isTrue();
		...
		// should work with other session
	...
	public void testFetchAccurateHybrisDataSourceAfterTenantRestart() throws JaloSecurityException, ConsistencyCheckException
	{
		...
		try
		{
			...
			final DataSource springBasedDataSourceBefore = Registry.getCoreApplicationContext()
			...
			final DataSource springBasedDataSourceAfter = Registry.getCoreApplicationContext()
			                                                      .getBean("dataSource", DataSource.class);
			Assert.assertSame("We should get the consistence data source vis spring factory  ", springBasedDataSourceAfter,
					Registry
