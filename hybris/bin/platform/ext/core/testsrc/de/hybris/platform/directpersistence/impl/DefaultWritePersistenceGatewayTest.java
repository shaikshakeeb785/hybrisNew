...
public class DefaultWritePersistenceGatewayTest extends ServicelayerTransactionalBaseTest
{
	...
	public void shouldStoreTestItemWithUnlocalizedOnlyProperties()
	{
		...
		final InsertRecord insertRecord = new InsertRecord(pk, ITEM_TYPE,
				Sets.newHashSet(new PropertyHolder(XXX_PROPERTY, "some value")));
	...
	{
		// given
		final List<PK> pks = IntStream.range(0, 5).mapToObj(i -> generatePkForCode(ITEM_TYPE)).collect(Collectors.toList());
		pks.stream().map(pk -> new InsertRecord(pk, ITEM_TYPE, Sets.newHashSet(new PropertyHolder(XXX_PROPERTY, "some value"))))
		   .forEach(insertRecord -> changeSet.add(insertRecord));
		// when
		writePersistenceGateway.persist(changeSet);
	...
	public void shouldStoreTestItemWithUnlocalizedAndLocalizedProperties()
	{
		...
		final InsertRecord insertRecord = new InsertRecord(pk, ITEM_TYPE,
				Sets.newHashSet(new PropertyHolder(XXX_PROPERTY, "some value")),
				mapOf(new Locale("en"), Sets.newHashSet(
						new PropertyHolder(TEST_PROPERTY2_PROPERTY, "english val")),
						new Locale("de"),
						Sets.newHashSet(new PropertyHolder(TEST_PROPERTY2_PROPERTY,
								"german val"))));
		changeSet.add(insertRecord);
	...
	public void shouldStoreTestItemWithUnlocalizedPropertiesAndUnlocalizedPropsValues()
	{
		...
		final InsertRecord insertRecord = new InsertRecord(pk, ITEM_TYPE,
				Sets.newHashSet(new PropertyHolder(XXX_PROPERTY, "some value"),
						new PropertyHolder(PROP_PROPERTY, "prop value")));
		changeSet.add(insertRecord);
	...
	public void shouldStoreTestItemWithUnlocalizedPropertiesAndLocalizedPropsValues()
	{
		...
		final PK pk = generatePkForCode(ITEM_TYPE);
		final InsertRecord insertRecord = new InsertRecord(pk, ITEM_TYPE,
				Sets.newHashSet(new PropertyHolder(XXX_PROPERTY, "some evil value")),
				mapOf(new Locale("en"), Sets.newHashSet(
						new PropertyHolder(PROP2_PROPERTY, "english val")),
						new Locale("de"),
						Sets.newHashSet(
								new PropertyHolder(PROP2_PROPERTY, "german val"))));
		changeSet.add(insertRecord);
