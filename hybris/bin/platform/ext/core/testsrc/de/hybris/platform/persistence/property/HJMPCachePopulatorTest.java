...
public class HJMPCachePopulatorTest extends ServicelayerBaseTest
{
	...
	{
		final Optional<CacheRegion> typesystemCacheRegion = cacheController.getRegions()
		                                                                   .stream()
		                                                                   .filter(r -> r.getName()
		                                                                                 .equals("typesystemCacheRegion"))
		                                                                   .findFirst();
		typesystemCacheRegion.ifPresent(CacheRegion::clearCache);
	}
	...
	private int getTypeSystemCacheRegionSize()
	{
		final Optional<CacheRegion> typesystemCacheRegion = cacheController.getRegions()
		                                                                   ...
		                                                                   .filter(r -> r.getName()
		                                                                                 .equals("typesystemCacheRegion"))
		                                                                   .findFirst();
		return typesystemCacheRegion.map(cacheRegion -> cacheRegion.getAllKeys().size()).orElse(0);
	}
}
