...
public class PasswordEncoderTest extends HybrisJUnit4TransactionalTest
{
	...
	public void testEncoders()
	{
		...
		try
		{
			...
			/* conv-log */
			log.debug("encoder class is " + enc.getClass().getName());
			String encoded = null;
			/* conv-log */
			log.debug("encoding  'test' " + (encoded = enc.encode(Constants.USER.ADMIN_EMPLOYEE, "nimda")));
			/* conv-log */
			log.debug("decoding  '" + encoded + "' " + enc.decode(encoded));
		}
		...
		{
			/* conv-log */
			log.debug("default encoder cannot decode password !!!");
		}
	}
