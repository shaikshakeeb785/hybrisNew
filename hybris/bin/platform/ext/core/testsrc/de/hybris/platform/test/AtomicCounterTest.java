...
public class AtomicCounterTest
{
	...
	private void testMultiThreadedCounter(final AtomicCounter counter, final int THREADS, final int RANGE)
	{
		...
		final int expectedCounterValue = expectOverflow ? counter.getCurrent() + RANGE - counter.getEndValue()
				: counter.getCurrent() + RANGE;

		final TestThreadsHolder<Runnable> threadsHolder = new TestThreadsHolder<>(THREADS,
				new de.hybris.platform.test.RunnerCreator<Runnable>()
				{
					@Override
					public Runnable newRunner(
							final int threadNumber)
					{
						return createRunnable(THREADS,
								threadNumber,
								RANGE, counter,
								gotOverflow,
								gotError);
