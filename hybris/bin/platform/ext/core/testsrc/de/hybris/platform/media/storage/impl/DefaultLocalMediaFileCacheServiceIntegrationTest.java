...
public class DefaultLocalMediaFileCacheServiceIntegrationTest extends ServicelayerBaseTest
{
	...
	private final PropertyConfigSwitcher cacheSizeConfigSwitcher = new PropertyConfigSwitcher(
			"media.default.local.cache.maxSize");
	...
	{
		final Optional<CacheRegion> mediaCacheRegion = Iterables.tryFind(cacheController.getRegions(),
				new Predicate<CacheRegion>()
				{

					@Override
					public boolean apply(
							@Nullable final CacheRegion input)
					{
	...
	{
		try (InputStream stream1 = localMediaFileCacheService.storeOrGetAsStream(folderConfig,
				MEDIA_LOCATION_IN_STORAGE_WITH_SPECIAL_CHARACTERS_AFTER_ENCODING,
				streamGetterWithSpecialCharacters))
		{
			verify(streamGetterWithSpecialCharacters, times(1)).getStream(folderConfig,
					MEDIA_LOCATION_IN_STORAGE_WITH_SPECIAL_CHARACTERS_AFTER_ENCODING);
		}
