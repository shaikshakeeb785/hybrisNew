...
public class Europe1PriceFactoryConcurrentTest extends HybrisJUnit4Test
{
	...
	public void testConcurentCacheCheck()
	{
		...
		final TestThreadsHolder<PriceFactoryRunner> randomAccessHolder = new TestThreadsHolder<>(//
				THREADS, //
				new de.hybris.platform.test.RunnerCreator<PriceFactoryRunner>()
				{

					@Override
					public PriceFactoryRunner newRunner(
							final int threadNumber)
					{

						if ((threadNumber % 2) == 0)
						{
							return new IsCachingPriceFactoryRunner(
									factory);
						}
						else
						{
							return new ClearCachingPriceFactoryRunner(
									factory);
						}
	...
	static private abstract class PriceFactoryRunner implements Runnable
	{
		...
		public void run()
		{
			try
			{
				...
				final List<Exception> recordedErrorTurns = new LinkedList<>();
