...
public class ConfigCartIntegrationTest extends CPQFacadeLayerTest
{
	...
	public void testSavedCartRecover() throws CommerceCartModificationException, CommerceSaveCartException
	{
		...
		assertTrue("Wrong price, expected " + CART_ENTRY_BASE_PRICE + " but was: " + cartEntryData.getBasePrice().getValue(),
				cartEntryData.getBasePrice().getValue().compareTo(CART_ENTRY_BASE_PRICE) == 0);
	...
	public void testConfigureFromCartForVariants() throws Exception
	{
		...
		importCsv("/sapproductconfigservices/test/sapProductConfig_stock_testData.impex", "utf-8");
		// in case sapproductconfigintegration is present we need an additional impex file
		importCsvIfExist("/sapproductconfigintegration/test/sapProductConfig_changeableVariants_testData.impex", "utf-8");
