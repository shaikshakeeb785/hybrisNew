		...
		implements Populator<ConfigModel, List<AbstractOrderEntryProductInfoModel>>
{
	...
	public void populate(final ConfigModel source, final List<AbstractOrderEntryProductInfoModel> target)
	{
		...
		final List<CsticGroupModel> csticGroups = rootInstance.getCsticGroups();
		if (csticGroups.isEmpty())
		{
			// this handles special case were cstics are not attached to groups, but directly to the instance
			// we could achieve the same by calling rootInstance.retrieveCsticGroupsWithCstics() instead of rootInstance.getCsticGroups()
			// however this would lead to a breaking change in method processGroupForCartDisplay, hence we manually check this here
			createDummyGroupForDircetlyAttachedCstics(rootInstance, csticGroups);
		}

		for (final CsticGroupModel currentGroup : csticGroups)
		{
	...
	protected void createDummyGroupForDircetlyAttachedCstics(final InstanceModel rootInstance,
			final List<CsticGroupModel> csticGroups)
	{
		final CsticGroupModelImpl dummyGroup = new CsticGroupModelImpl();
		dummyGroup.setName(InstanceModel.GENERAL_GROUP_NAME);
		dummyGroup.setCsticNames(rootInstance.getCstics().stream().map(cstic -> cstic.getName()).collect(Collectors.toList()));
		csticGroups.add(dummyGroup);
	}
