...
public class IntervalInDomainHelperImpl implements IntervalInDomainHelper
{
	...
	public String formatNumericInterval(final String interval)
	{
		...
		else
		{
			...
			if (match.find())
			{
				formattedInterval = match.group(INTERVAL_FROM_GROUP) + getValueFormatTranslator().formatNumeric(match.group(INTERVAL_TO_GROUP).trim());
			}
		}
	...
	{
		return Localization.getLocalizedString("type.ProductConfiguration.IntervalValue.conflict", new Object[] { value, interval });
	}
