...
public class DefaultItemSynchronizationFacadeTest
{
	...
	public void setup()
	{
		...
		doAnswer(new Answer<ItemModel>()
		{
			@Override
			public ItemModel answer(final InvocationOnMock invocation) throws Throwable
			{
				final Supplier<ItemModel> supplier = (Supplier) invocation.getArguments()[0];
				return supplier.get();
			}
	...
	{
		final SyncItemStatusConfig config = Mockito.mock(SyncItemStatusConfig.class);
		final ItemSynchronizationData itemDataInvalid = Mockito.mock(ItemSynchronizationData.class);

		synchronizationStatusFacade.getSynchronizationItemStatus(syncRequestData, itemDataInvalid, config);
	}

	@Test(expected = IllegalArgumentException.class)
	public void shouldFailPerformItemSynchronizationWhenOneItemModelIsNotFound()
	{
		final SyncConfig config = Mockito.mock(SyncConfig.class);
		final ItemSynchronizationData itemDataInvalid = Mockito.mock(ItemSynchronizationData.class);
		when(synchronizationData.getItems()).thenReturn(asList(itemData2, itemData3, itemDataInvalid));

		synchronizationStatusFacade.performItemSynchronization(syncRequestData, synchronizationData, config);
	}
