...
<c:choose>
	<c:when test="${fn:toUpperCase(currentLanguage.getIsocode()) eq 'ZH'}">
		...
		<br><c:if test="${not empty address.city && not empty address.city.name}">
			${fn:escapeXml(address.city.name)}&nbsp;
		</c:if>
		<c:if test="${empty address.city || empty address.city.name}">
			${fn:escapeXml(address.town)}&nbsp;
		</c:if>
		<c:if test="${not empty address.cityDistrict && not empty address.cityDistrict.name}">
			${fn:escapeXml(address.cityDistrict.name)}
		</c:if>
	...
	<c:otherwise>
		...
		<br><c:if test="${not empty address.cityDistrict && not empty address.cityDistrict.name}">
			${fn:escapeXml(address.cityDistrict.name)}&nbsp;
		</c:if>
		<c:if test="${not empty address.city && not empty address.city.name}">
			${fn:escapeXml(address.city.name)}&nbsp;
		</c:if>
		<c:if test="${empty address.city || empty address.city.name}">
			${fn:escapeXml(address.town)}
