...
public class OrderHistoryServiceTest extends ServicelayerTransactionalTest
{
	...
	protected OrderModel placeOrder(final UserModel user) throws Exception //NOPMD
	{
		...
		// to avoid errors regarding non-unique cart/order codes (BIT-3733,BIT-3755)
	...
	public void testEntries() throws Exception
	{
		...
		Assert.assertTrue("Order history entry doesn't have relation to the proper order",
				entriesAfter.get(entriesAfter.size() - 1).getOrder().equals(order));
	...
	public void testOrderCloning()
	{
		...
		final int digits = currItem.getDigitsAsPrimitive();
		...
		try
		{
			...
			calculationService.calculate(originalOrder);
		}
		catch (final CalculationException e)
		{
			throw new SystemException("Could not calculate order [" + originalOrder.getCode() + "] due to : " + e.getMessage(), e);
		}
		finally
		{
		...
		assertEquals(commonI18NService.roundCurrency(5 * 2.55, digits), entry1.getTotalPrice().doubleValue(), 0.00001);

		assertEquals(Boolean.TRUE, entry2.getCalculated());
		assertEquals(5.33, entry2.getBasePrice().doubleValue(), 0.00001);
		assertEquals(commonI18NService.roundCurrency(10 * 5.33, digits), entry2.getTotalPrice().doubleValue(), 0.00001);

		assertEquals(Boolean.TRUE, originalOrder.getCalculated());

		assertEquals(commonI18NService.roundCurrency(10 * 5.33, digits) + commonI18NService.roundCurrency(5 * 2.55, digits),
				originalOrder.getTotalPrice().doubleValue(), 0.00001);
		...
		assertEquals(commonI18NService.roundCurrency(5 * 2.55, digits), e1Clone.getTotalPrice().doubleValue(), 0.00001);

		assertEquals(Boolean.TRUE, e2Clone.getCalculated());
		assertEquals(5.33, e2Clone.getBasePrice().doubleValue(), 0.00001);
		assertEquals(commonI18NService.roundCurrency(10 * 5.33, digits), e2Clone.getTotalPrice().doubleValue(), 0.00001);
		assertEquals(commonI18NService.roundCurrency(10 * 5.33, digits), e2Clone.getTotalPrice().doubleValue(), 0.00001);

		assertEquals(Boolean.TRUE, clone.getCalculated());
		assertEquals(commonI18NService.roundCurrency(10 * 5.33, digits) + commonI18NService.roundCurrency(5 * 2.55, digits),
				clone.getTotalPrice().doubleValue(), 0.00001);
