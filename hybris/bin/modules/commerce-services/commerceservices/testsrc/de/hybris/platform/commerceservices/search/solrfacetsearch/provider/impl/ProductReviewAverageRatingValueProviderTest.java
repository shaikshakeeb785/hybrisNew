...
public class ProductReviewAverageRatingValueProviderTest extends PropertyFieldValueProviderTestBase
{
	...
	public void testWhenIndexPropertyIsLocalized() throws FieldValueProviderException
	{
		...
		given(fieldNameProvider.getFieldNames(indexedProperty, TEST_EN_LANG_CODE))
				.willReturn(singletonList(TEST_PROP_FIELD_NAME_EN));
		given(fieldNameProvider.getFieldNames(indexedProperty, TEST_DE_LANG_CODE))
				.willReturn(singletonList(TEST_PROP_FIELD_NAME_DE));
	...
	{
		given(Boolean.valueOf(indexedProperty.isLocalized())).willReturn(Boolean.FALSE);
		given(fieldNameProvider.getFieldNames(Matchers.<IndexedProperty> any(), eq(nullString)))
				.willReturn(singletonList(TEST_PROP_FIELD_NAME));
	...
	{
		((FieldValueProvider) getPropertyFieldValueProvider()).getFieldValues(indexConfig, indexedProperty, new CatalogModel());
	}
}
