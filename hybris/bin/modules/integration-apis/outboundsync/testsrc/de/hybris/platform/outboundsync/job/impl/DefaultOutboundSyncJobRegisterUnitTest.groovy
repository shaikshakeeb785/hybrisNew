...
class DefaultOutboundSyncJobRegisterUnitTest extends Specification {
    ...
    def 'persists and unregisters the job when the job finishes'() {
        ...
        then:
        1 * modelService.refresh(jobModel)
        then:
        1 * jobModel.setStatus(changedState.cronJobStatus)
        1 * jobModel.setResult(changedState.cronJobResult)
        1 * jobModel.setStartTime(changedState.startTime)
        1 * jobModel.setEndTime(changedState.endTime)
        1 * jobModel.setRequestAbort(null)
        then:
        1 * modelService.save(jobModel)
    ...
    def 'persists but keeps the job registered when the job is not finished'() {
        ...
        1 * jobModel.setResult(changedState.cronJobResult)
        1 * jobModel.setStartTime(changedState.startTime)
        0 * jobModel.setEndTime(changedState.endTime)
        0 * jobModel.setRequestAbort(null)
        then:
        1 * modelService.save(jobModel)
    ...
    def 'unregisters a finished job state even when it failed to persist'() {
        ...
        modelService.refresh(jobModel) >> { throw new ModelPersistenceException('') }
