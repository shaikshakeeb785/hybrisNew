...
class OutboundSyncStateUnitTest extends Specification {
    ...
    private static final def POST_ABORT_ITEMS = 1
    ...
    def "exception is thrown when #fieldCondition"() {
        ...
        'post abort items is negative' | 0     | 0         | 0      | -1
    }

    @Test
    @Unroll
    def "job status is RUNNING when total number of items is unknown and #field is applied"() {
        given: 'total number of successful items, error items and post abort items is less then total items count'
    ...
    @Test
    @Unroll
    def "job status is #status when all items processed and post abort items #condition"() {
        given: 'total number of successful items, error items and post abort items is less then total items count'
        def state = initialOutboundSyncState()
                .withTotalItems(TOTAL_ITEMS)
                .withSuccessCount(3)
                .withErrorCount(2)
                .withUnprocessedCount(aborted)
                .build()
        ...
        where:
        condition     | aborted | status
        'present'     | 1       | CronJobStatus.ABORTED
        'not present' | 0       | CronJobStatus.FINISHED
    }
    ...
    def "job result is #result when #condition"() {
        ...
        'no items processed'              | 0       | 0     | CronJobResult.UNKNOWN
        'no error items present'          | 1       | 0     | CronJobResult.SUCCESS
        'at least one error item present' | 1       | 1     | CronJobResult.ERROR
    }
    ...
    def "state changed is #res when #condition"() {
        ...
        'aborted count incremented' | false | initialOutboundSyncState()                     | initialOutboundSyncState().withUnprocessedCount(1)
    ...
    private static OutboundSyncState.Builder outboundSyncStateBuilder() {
        initialOutboundSyncState()
                ...
                .withUnprocessedCount(POST_ABORT_ITEMS)
