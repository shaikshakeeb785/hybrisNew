...
public final class EditorUtils
{
	...
	/**
	 * Find a matching Treeitem in a given parent Treeitem's Treechildren by AttributeDescriptorModel qualifier.
	 *
	 ...
	 */
	public static boolean markRowsWithDuplicateNames(Collection<Listitem> items,
	                                                 Map<String, List<AbstractListItemDTO>> duplicateEntries)
	{
		boolean hasHighlight = false;
		items.forEach(item -> UITools.removeSClass(item, "integration-object-attribute-name-conflict"));
		if (duplicateEntries != null)
		{
			for (List<AbstractListItemDTO> listItemDTOS : duplicateEntries.values())
			{
				for (AbstractListItemDTO entry : listItemDTOS)
				{
	...
	{
		for (AbstractListItemDTO dto : dtos)
		{
	...
	private static void assignEditModeRules(Listitem listitem, Checkbox uniqueCheckbox, Checkbox autocreateCheckbox,
	                                        Menupopup menuPopup)
	{
	...
	{
		final List<ListItemAttributeDTO> attributes = item.getAttributes().stream()
		                                                  .map(attr -> toListItemAttributeDTO(readService, attr))
		                                                  .collect(Collectors.toList());
		final List<ListItemClassificationAttributeDTO> classificationAttributes = item
