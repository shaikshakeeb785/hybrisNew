...
class ItemTypeDescriptorUnitTest extends Specification {
    ...
    def "getAttributes() returns attributes defined in the item model"() {
        ...
        expect:
        descriptor.attributes.size() == itemModel.attributes.size() + itemModel.classificationAttributes.size()
        descriptor.attributes.collect({ it.getAttributeName() }).containsAll(['code', 'name', 'weight', 'size'])
    }
