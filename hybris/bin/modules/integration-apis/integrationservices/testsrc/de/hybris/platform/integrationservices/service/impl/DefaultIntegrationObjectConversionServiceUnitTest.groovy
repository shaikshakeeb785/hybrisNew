...
class DefaultIntegrationObjectConversionServiceUnitTest extends Specification {
    ...
    def setup() {
        ...
        conversionService.descriptorFactory = descriptorFactory
    ...
    def "populate result map when item matches an integration object item type"() {
        ...
        and: 'a descriptor is created for the integration object model'
        def typeDescriptor = Stub(TypeDescriptor) {
            isInstance(item) >> true
        }
        descriptorFactory.createIntegrationObjectDescriptor(ioModel) >> Stub(IntegrationObjectDescriptor) {
            getItemTypeDescriptor(item) >> Optional.of(typeDescriptor)
        }
