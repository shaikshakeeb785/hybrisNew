...
@UnitTest
class NullableAttributePersistenceContextValidatorUnitTest extends Specification {
    private def validator = new NullableAttributePersistenceContextValidator()

    @Test
    def "integration item is not present"() {
        when:
        validator.validate(persistenceContext(null))
    ...
    def "non-nullable #attributeType attribute with null value results in exception thrown"() {
        ...
        then:
        thrown MissingNavigationPropertyException
