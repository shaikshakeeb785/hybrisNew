...
class ReadHandlerUnitTest extends Specification {
    ...
    def "fail to read with #exception"() {
        ...
        where:
        exception << [Stub(PropertyNotFoundException),
                      Stub(ItemNotFoundException),
                      Stub(OData2ServicesException),
                      Stub(IntegrationAttributeException),
                      Stub(TypeAttributeDescriptorNotFoundException),
                      Stub(FilterByClassificationAttributeNotSupportedException),
                      Stub(IntegrationKeyNestedFilteringNotSupportedException),
                      Stub(TypeAccessPermissionException)]
    }
    ...
    def "fail to read with RuntimeException"() {
        ...
        and:
        entityReaderRegistry.getReader(_ as UriInfo) >> Stub(EntityReader) {
            read(_) >> { throw new RuntimeException('IGNORE - testing exception') }
        }
