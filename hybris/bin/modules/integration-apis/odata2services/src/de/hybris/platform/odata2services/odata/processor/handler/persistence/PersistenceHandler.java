...
public class PersistenceHandler implements ODataProcessorHandler<PersistenceParam, ODataResponse>
{
	...
	protected ODataEntry persist(final StorageRequest storageRequest) throws EdmException
	{
		...
		catch (final IntegrationAttributeException | PersistenceRuntimeApplicationException | ItemNotFoundException | LocaleNotSupportedException | ItemSearchRequestValidationException | TypeAccessPermissionException | NonUniqueItemFoundException e)
		{
			throw e;
		}
		catch (final RuntimeException e)
	 ...
	 *
	 * @param param          Parameters from the request
	 * @param storageRequest Storage request contains the information on how to create the response
	 ...
	 */
	protected ODataResponse.ODataResponseBuilder buildResponse(final PersistenceParam param, final StorageRequest storageRequest,
	                                                           final ODataEntry persistedEntry) throws ODataException
	{
		...
		return getODataResponseBuilder(writeEntry(param, persistedEntry, writeProperties))
				.header(HttpHeaders.CONTENT_LANGUAGE, storageRequest.getAcceptLocale().toString());
	}
