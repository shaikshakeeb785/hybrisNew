...
public class ControllerExceptionHandlerUnitTest
{
	...
	{
		whenHandlingHttpRequestMethodNotSupportedException();

		verifyODataResponse(HttpStatusCodes.NOT_IMPLEMENTED);
	}

	@Test
	public void testHandlerUpsertIntegrationObjectWithClassificationException() throws Exception
	{
		final var exception = new UpsertIntegrationObjectWithClassificationAttributeException("patch");

		whenHandlingUpsertIntegrationObjectWithClassificationException(exception);

		verifyODataResponse(HttpStatusCodes.NOT_IMPLEMENTED, "Executing a PATCH request on an Integration Object configured with classification attributes is not currently supported.");
	}

	@Test
	public void testHandlerTypeAccessPermissionException() throws Exception
	{
		final var exception = new TypeAccessPermissionException("Order", "read");

		whenHandlingTypeAccessPermissionException(exception);

		verifyODataResponse(HttpStatusCodes.FORBIDDEN, "The user lacks read privileges for accessing the Order type.");
	}
	...
	private void whenHandlingHttpRequestMethodNotSupportedException()
	{
		captureODataResponse();
		controllerExceptionHandler.handleMethodNotSupportedException(getRequest());
	}

	private void whenHandlingUpsertIntegrationObjectWithClassificationException(final UpsertIntegrationObjectWithClassificationAttributeException exception)
	{
		captureODataResponse();
		controllerExceptionHandler.handleUpsertIntegrationObjectWithClassificationException(getRequest(), exception);
	}

	private void whenHandlingTypeAccessPermissionException(final TypeAccessPermissionException exception)
	{
		captureODataResponse();
		controllerExceptionHandler.handleTypeAccessPermissionException(getRequest(), exception);
	}
