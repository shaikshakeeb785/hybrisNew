...
public class AddOnCoreDataImportedEventListener extends AbstractEventListener<CoreDataImportedEvent>
{
	...
	{
		boolean productFilesImported = false;
		String productCatalogId = StringUtils.EMPTY;
		try
		{
			productCatalogId = importData.getProductCatalogName() + "ProductCatalog";
			final CatalogModel productCatalog = getCatalogService().getCatalogForId(productCatalogId);
			...
			productFilesImported = getAddonConfigDataImportService().executeImport(parameterData.getConfigExtensionName(),
		...
		{
			LOG.error("Product Catalog[" + productCatalogId + "] cannot be found out", e);
		}
